...
class ESPCN4x(nn.Module):
    def __init__(self):
        ...
        self.c = conv_block(self.nf * self.num_modules, self.nf, kernel_size=1, act_type='lrelu')

        self.LR_conv = conv_layer(self.nf, self.nf, kernel_size=3)

        self.upsampler = pixelshuffle_block(self.nf, self.out_nc, upscale_factor=self.upscale)
...
def conv_block(
    ...
    return nn.Sequential(*([p] if p else []) + [c] + ([n] if n else []) + ([a] if a else []))
